<template>
	<view class="container" :style="[{'height': height}]">
		<editor class="ql-container" :placeholder="placeholder" :show-img-size="false" :show-img-toolbar="false"
			:show-img-resize="false" @ready="onEditorReady" id="editor" @statuschange="statuschange" @blur="editBlur"
			ref="editot"></editor>
		<!-- 操作工具 -->
		<view class="tool-view">
			...
			<view class="tool">
				...
				<jinIcon class="single" type="&#xeb8a;" font-size="44rpx" title="设置" @click="submit"></jinIcon>
			</view>
		</view>
	</view>
</template>
		...
		methods: {
				...
				uni.chooseImage({
					...
					success: async (res) => {
						...
						for (let temp of tempFilePaths) {
							// 图片上传服务器
							const pic_res = await commonApi.uploadImg(temp);
							console.log(88, pic_res.data);

							this.editorCtx.insertImage({
								src: pic_res.data, // 此处需要将图片地址切换成服务器返回的真实图片地址
								alt: '图片',
								success: function(e) {
									uni.hideLoading();
								}
			...
			async editFocus(e) {
				...
				uni.getSelectedTextRange({
					success: res => {
						console.log('getSelectedTextRange res', e.start, e.end)
					}
				})
